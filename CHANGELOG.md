...
- **`login --browser`** (#244) — OAuth2 authorization-code + PKCE login through the system browser, alongside the existing device-code flow. Binds to `127.0.0.1` only, never logs the authorization code or tokens, times out the local listener, and supports `--no-open` (print the URL instead of launching a browser), `--localhost-port`, and `--callback-timeout`. `src/lib/pkce.ts`, `src/lib/browser-login.ts`.
- **Identity profiles and wrong-account guardrails** (#245) — `m365-agent-cli profiles list/show/set-default/delete` bind a friendly name to a token-cache identity slot and record the last-verified signed-in account; `set-default` becomes the fallback identity for every command. New root flags `--require-identity <upn>` and `--as-delegate-of <upn>` (paired with `--mailbox`) fail closed before any command runs unless the signed-in identity matches — an unverifiable identity is treated as a mismatch, not a pass-through. Both `login` flows gained `--identity`/`--force-identity-switch`: re-logging into an already-verified identity slug under a different account is refused unless forced. `src/lib/identity-profiles.ts`, `src/lib/identity-guard.ts`, `src/lib/login-identity-binding.ts`, `src/commands/profiles.ts`.
- **`doctor` / `doctor --redacted-bundle`** (#246) — non-secret diagnostic summary safe to attach to an issue: CLI/Node/platform versions, config/cache file presence/size/mtime (never contents), classified auth failure, capability summary. `--json` for machine-readable output; `--redacted-bundle [path]` writes a real `.zip` (hand-rolled minimal ZIP writer, no new dependency, cross-validated against `unzip` and Python's `zipfile`), or `--format dir --output <path>` for a plain directory. Every field passes through a deep-redaction pass (`src/lib/redact.ts`) as defense in depth. `src/lib/doctor-bundle.ts`, `src/lib/minimal-zip.ts`, `src/commands/doctor.ts`.
- **`readiness --json`** (#247) — stable, versioned JSON contract for "can I safely send mail/read calendar right now?": `ready`, `signedInAs`, `authHealth`, `cacheHealth`, `capabilities`/`missingCapabilities` (via repeatable `--require <capability>`, e.g. `mail.read`, `mail.send`, `calendar.read`), optional `mailboxAccess` (via `--mailbox`), optional `--expect-identity` mismatch check, and — whenever not ready — `recommendedAction`/`safeCommand`. Exits `0` whenever the CLI itself ran successfully; readiness lives in the JSON, not the exit code. `src/commands/readiness.ts`.
