...
- Interactive login: `m365-agent-cli login` (device code) or `login --browser` (authorization-code + PKCE through the system browser); tokens land in `.env` / caches. **Delegated Graph scopes** (Entra app + CLI features): **`docs/GRAPH_SCOPES.md`**. For **another user’s** mail/calendar (`--mailbox`), include **Mail/Calendars \*.Shared**; for **`find`**, **Places**/**`rooms`**, add scopes listed there; re-`login` after changing the app registration.
- **Unattended / headless login & MFA enrollment:** automate the device-code step (`login --json` for NDJSON events), and bootstrap a software **TOTP** (optionally signing in with a **Temporary Access Pass** when the tenant requires MFA to register, or the account already has MFA) — see **`docs/UNATTENDED_LOGIN.md`** and **`examples/unattended-login/`** (`enroll-totp.mjs`, `enroll.sh`). A TAP can't set a password and TOTP is only a second factor, so a first-factor password must still exist for refreshes.
...
- **Named identity profiles:** `m365-agent-cli profiles list/show/set-default/delete` — `set-default` becomes the fallback `--identity` everywhere. Wrong-account guardrails: root flags **`--require-identity <upn>`** / **`--as-delegate-of <upn>` (with `--mailbox`)** fail closed before any command runs if the signed-in identity doesn't match.
- **Before doing mail/calendar work, prefer `m365-agent-cli readiness --json`** over guessing from a failed call — it returns `ready`/`missingCapabilities`/`recommendedAction` in one shot. If something is broken, `m365-agent-cli auth repair --json` classifies the failure and gives one safe command; `doctor --redacted-bundle` produces a non-secret bundle for a bug report.
...
| Other | `login` (device code, or **`--browser`** for authorization-code + PKCE), `whoami`, `verify-token`, `serve` |
| Auth diagnostics (agents) | `readiness --json` — machine-readable `ready`/`missingCapabilities` contract; `auth repair [--json]` — classifies the failure (revoked/expired grant, MFA, missing creds, …) and gives one safe command; `doctor [--json\|--redacted-bundle]` — non-secret summary/shareable bundle; `profiles list/show/set-default/delete` — named identity profiles |
...
- If auth fails, run **`m365-agent-cli auth repair --json`** first — it classifies the failure (revoked/expired grant, interaction required, MFA, missing creds, tenant/client mismatch) and returns a `recommendedAction` + `safeCommand`, without printing any token material. Before starting mail/calendar work, **`readiness --json`** answers "can I do this right now" in one call (`ready`, `missingCapabilities`, `recommendedAction`). Wrong **identity** profile means wrong cache file — check `--identity`, or `m365-agent-cli profiles show` for the bound account. Treat `AADSTS50173` / `TokensValidFrom` / `invalid_grant` as **revoked refresh grant** (this is exactly what `auth repair` classifies it as) — stop and ask for interactive `login` (or `login --browser` if device-code MFA repeatedly fails); do not invent alternate mail backends. For a shareable, non-secret summary to attach to a bug report, use **`doctor --redacted-bundle`**.
- Refresh-token exchange is serialized per `--identity` (shared EWS/Graph RT). Concurrent CLI processes wait on `.refresh-{identity}.lock` under the config dir instead of racing Entra rotation. The lock **auto-heals** a stale holder (dead PID or >120s old), so **never delete a lock file by hand** between runs.
- **Driving `login` from a wrapper:** it is a **synchronous foreground poll** — keep the process **alive** until sign-in completes (it polls until the browser step finishes or the code expires, ~15 min); killing it early discards the pending device-code session even if the browser shows success. Verify after with **`whoami`** + **`verify-token --capabilities`**, not the wrapper's own success print. For unattended automation use **`login --json`** (NDJSON events on stdout: `device_code` / `authorization_url` / `authenticated` / `complete` / `error`) instead of scraping log text — full guide + adaptable example: **`docs/UNATTENDED_LOGIN.md`**, **`examples/unattended-login/`**. To **bootstrap a fresh account's software TOTP** headlessly (optionally via a **Temporary Access Pass** when the tenant requires MFA to register, or the account already has MFA), see that guide's *Setting up software TOTP* / *Automated first-time TOTP enrollment* sections plus **`examples/unattended-login/enroll-totp.mjs`** and **`enroll.sh`**.
