...
| `Tasks.ReadWrite` | Microsoft To Do for the signed-in user's own lists/tasks. For shared/delegated To Do, also configure `Tasks.Read.Shared` / `Tasks.ReadWrite.Shared`; Graph To Do `/users/{other}/todo/...` can still return `Invalid request` when the target user's To Do service or sharing relationship is not available through that endpoint. |
