...
export const roomsCommand = new Command('rooms')
  ...
  .action(
    async (options: { building?: string; capacity?: string; equipment?: string; json?: boolean; token?: string }) => {
      const hasFilters = !!(options.building || options.capacity || options.equipment);
      if (!hasFilters) {
        const result = await listPlaceRoomLists();
        if (!result.ok || !result.data) {
          if (options.json)
            console.log(JSON.stringify({ error: result.error?.message || 'Failed to list room lists' }, null, 2));
        ...
        if (lists.length === 0) console.log('  No room lists found or access denied.');
          ...
          for (const rl of lists) {
            console.log(`  \u{1F4CB} ${rl.displayName}`);
            if (rl.emailAddress) console.log(`     ${rl.emailAddress}`);
            console.log('');
          }
        return;
      }
      ...
      if (!result.ok || !result.data) {
        if (options.json)
          console.log(JSON.stringify({ error: result.error?.message || 'Failed to list rooms' }, null, 2));
      ...
      const fdesc = [
        options.building ? `building="${options.building}"` : null,
        filters.capacityMin !== undefined ? `capacity>=${filters.capacityMin}` : null,
        filters.equipment?.length ? `equipment=${filters.equipment.join(',')}` : null
      ]
        .filter(Boolean)
        .join(' | ');
      console.log(`\n\u{1F3E0}  Meeting Rooms (${rooms.length})${fdesc ? ` -- ${fdesc}` : ''}\n`);
      if (rooms.length === 0) {
        console.log('  No rooms match your filters, or access is denied.');
      } else {
        ...
        for (const [building, buildingRooms] of byBuilding) {
          console.log(
            `  ${building !== 'No Building' ? `\u{1F3D7}  ${building}` : '\u{1F3E0}  Unknown Building'} (${buildingRooms.length})`
          );
...
roomsCommand
  ...
  .action(
    async (options: { building?: string; capacity?: string; equipment?: string; json?: boolean; token?: string }) => {
      ...
      if (rooms.length === 0) {
        console.log('  No rooms match your filters.');
      } else {
        const byBuilding = new Map<string, Place[]>();
        for (const room of rooms) {
          const key = room.building || 'No Building';
...
roomsCommand
  ...
  .action(
    async (
      ...
      if (!result.ok || !result.data) {
        if (options.json)
          console.log(JSON.stringify({ error: result.error?.message || 'Failed to find rooms' }, null, 2));
      ...
      if (options.json) {
        console.log(
          JSON.stringify(
            {
              timeSlot: { start: start.toISOString(), end: end.toISOString() },
              filters,
      ...
      if (availableRooms.length === 0) {
        console.log('  \u{274C}  No available rooms found for this time slot.');
      } else {
